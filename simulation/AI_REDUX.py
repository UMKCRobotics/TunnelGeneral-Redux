...
class RobotAlg():
    ...
    def calibrate_x(self, side = "B"):  # calibrate on edge
        if self.DEBUG_MODE: print 'CALIBRATING X'
        if self.MAP.robotLoc[0] == 0:
            self.turn(0)
        elif self.MAP.robotLoc[0] == 6:
            self.turn(2)
        self.wait_till_done(self.sim_robot.goCalibrateIR(side))
        sleep(self.SLEEP_TIME)

    def calibrate_y(self, side = "B"):
    ...
    def get_demo_path(self):
        return ['A6','A5','A4','A3','A2','A1','B1',
                'B2','B3','B4','B5','B6','B7','C7',
                'C6','C5','C4','C3','C2','C1']
    ...
    def doStuff(self):
        ...
        self.goList = self.get_demo_path()
